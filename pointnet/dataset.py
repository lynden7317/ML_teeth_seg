...
class ShapeNetDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        try:
            ...
            choice_crown = np.random.choice(self.point_set_crown.shape[0], int(self.npoints/2), replace=True)
        try:
            choice_gum = np.random.choice(self.point_set_gum.shape[0], int(self.npoints/2), replace=False)
        except:
            choice_gum = np.random.choice(self.point_set_gum.shape[0], int(self.npoints/2), replace=True)
        #resample
        ...
        if self.data_augmentation:
            ...
            point_set += np.random.normal(0, 0.02, size=point_set.shape) # random jitter 0.02
    ...
    def __len__(self):
        if self.point_set_crown is None:
            return 0
        else:
            total = self.point_set_crown.shape[0] + self.point_set_gum.shape[0]
        
        return math.ceil(total/self.npoints)
